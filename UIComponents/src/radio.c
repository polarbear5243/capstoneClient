...
/*
 * @brief Function will be operated when radio's value is changed
...
{
	int value = 0;
	dlog_print(DLOG_INFO, LOG_TAG, "Selected Radio:%d", (int)data);
	value = elm_radio_value_get(obj);
	/* will print the value assigned to the currently selected radio */
	dlog_print(DLOG_INFO, LOG_TAG, "selected value for the group:%d", value);
}
...
/*
 * @brief Function to create radio object
 * @param[in] parent The Evas object which radio object is created on
 * @param[out] Evas_Object* The Evas object which is created
 */
static Evas_Object*
...
/*
 * @brief Function will be operated when main menu "Radio" is clicked
 * @param[in] data The data to be passed to the callback function
 * @param[in] obj The Evas object handle to be passed to the callback function
 * @param[in] event_info The system event information
 */
